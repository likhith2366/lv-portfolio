...
const ITEMS = [
  {
    label: 'Resume',
    href: '/Assets/resume/Likhith_Main_nyu.pdf',
    color: '#ef4444',
    icon: (
  ...
  {
    label: 'LinkedIn',
    href: 'https://www.linkedin.com/in/likhithvardhangoruputi',
    color: '#0a66c2',
    icon: (
  ...
  {
    label: 'GitHub',
    href: 'https://github.com/likhith2366',
    color: '#fff',
    icon: (
...
const DockIcon = ({ item, mouseX }) => {
  const ref = useRef(null);

  const dist = useTransform(mouseX, (val) => {
    if (!ref.current || val === -1000) return MAGNIFY_DISTANCE;
    const rect = ref.current.getBoundingClientRect();
    const center = rect.left + rect.width / 2;
    return Math.abs(val - center);
  });

  const sizeRaw = useTransform(dist, [0, MAGNIFY_DISTANCE], [MAX_SIZE, BASE_SIZE]);
  const size = useSpring(sizeRaw, SPRING_CONFIG);

  const liftRaw = useTransform(dist, [0, MAGNIFY_DISTANCE], [MAX_LIFT, 0]);
  const y = useSpring(liftRaw, SPRING_CONFIG);
